...
> ## ⚠️ Прочтите это до того, как доверитесь графу
>
> Все числа в этой карте проверены **по исходникам**, а не взяты из индексатора. У графа
> codebase-memory обнаружены систематические ошибки: он выдаёт за маршруты строки из тестов,
> завышает связность на универсальных именах методов (`create`, `get`, `count`), неверно
> определяет точки входа и молча пропускает часть файлов.
>
> Разбор с доказательствами и таблица «какой вопрос — какой источник истины» —
> в разделе [**Чему в графе нельзя верить**](#чему-в-графе-нельзя-верить)
> в конце документа. **Не стройте выводы на выводе индексатора, не сверившись с ним.**
>
> Если карта разойдётся с кодом — верен код.

## Содержание

- [Что это](#что-это) · [Слой данных](#слой-данных) · [Маршрутизация](#маршрутизация)
- [Контроллеры](#контроллеры) · [Сервисы](#сервисы--фактическое-ядро-системы) · [Обвязка](#обвязка)
- [Консольные команды](#консольные-команды-25) · [Тесты](#тесты) · [Грабли](#грабли)
- ⚠️ [**Чему в графе нельзя верить**](#чему-в-графе-нельзя-верить)
...
**Точечные файлы Apache сам не закрывает** — только `.ht*`. Из-за этого `public/.user.ini`
с лимитами PHP отдавался наружу с кодом 200. Закрыт `FilesMatch` плюс дублирующим
правилом `rewrite` (до Apache запрос может не дойти — см. пункт выше). Исключение для
`.well-known` задано **явной строкой**, а не отрицательным просмотром вперёд: когда
каталога нет на диске, Apache подставляет его имя как имя файла и шаблон срабатывает
на самом `.well-known`, ломая продление сертификата.

**Как отличать, кто ответил.** Размер тела ответа: **199 байт — Apache**, **153 — nginx**,
**21 393 — страница 404 Laravel**. Быстрый способ понять, дошёл ли запрос до приложения.

**`/.well-known/` перехватывает nginx** и до Apache он не доходит вовсе — проверено
одинаковым ответом по http и https независимо от прав и правил `.htaccess`. Выпуск
и продление SSL на этом хостинге от содержимого `public/` не зависят.
...
⚠️ **Ключевой раздел документа.** Всё ниже проверено на этом проекте в августе 2026:
каждое утверждение подтверждено чтением исходника, а не предположением. Если вы пришли
сюда по ссылке из шапки — вы там, где нужно.
