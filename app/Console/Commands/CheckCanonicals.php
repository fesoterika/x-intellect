 ...
 *
 * SeoService::fillDefaults выставляет canonical один раз (`??=`), поэтому после
 * переезда страницы в другой корневой раздел он остаётся на прежнем адресе и
 * спорит с собственным 301: поисковик получает канонический адрес, который сам
 * же и редиректит. PageObserver чинит это для новых переездов — команда
 * подчищает накопленное.
 *
