...
class ApplySiteStructure2026 extends Command
{
    ...
    private array $memorialPages = [
        ...
        // Годовщины 2014–2016 и книга памяти — из Wayback (в слепке 2015 их нет).
        // «Светлая память Александру Глазу» (2016) — отдельный материал, а не
        // повтор «СВЕТЛАЯ ПАМЯТЬ АЛЕКСАНДРУ ГЛАЗУ» (2013): у старого сайта это
        // разные записи, третья годовщина и прощание.
        'Годовщина со дня ухода Александра Георгиевича Глаза, основателя сообщества и сайта Х-ИНТЕЛЛЕКТ',
        'Исполнилось 2 года со дня ухода из земного воплощения Александра Георгиевича Глаза, основателя сообщества и сайта Х-ИНТЕЛЛЕКТ',
        'Светлая память Александру Глазу',
        'ВНИМАНИЕ! Книга -памяти об Александре Глаз.',
    ];
    ...
    private array $projectRules = [
        // Название серии на старом сайте — через тире: «Человек — Земля — Космос 2014»
        ['chelovek-zemlya-kosmos', 'Человек — Земля — Космос', 9, '/человек[\s,—–-]+земл[яю][\s,—–-]+космос/ui'],
    ...
    public function handle(): int
    {
        ...
        // 2в. Страницы-указатели старого сайта: на новом их роль играют листинги
        // подразделов, поэтому переносить нечего — нужен только 301.
        if ($digests->exists && $memorial->exists) {
            $this->legacyIndexRedirect('/digests', $digests->url(), 'Дайджесты X-Intellect', $dry);
            $this->legacyIndexRedirect('/magx', $memorial->url(), 'Памяти А. Глаза', $dry);
        }

        // «О сайте» перенесена с корня старого сайта, поэтому её прежний плоский
        // адрес не выводится из source_url — прописываем 301 явно.
        if ($about = Page::where('slug', 'o-sajte-x-intellect')->first()) {
            $this->legacyIndexRedirect('/o-sajte-x-intellect', $about->url(), 'О сайте X-INTELLECT', $dry);
        }
    ...
    /** 301 со старого адреса-указателя на заменивший его листинг или страницу. */
    private function legacyIndexRedirect(string $from, string $to, string $what, bool $dry): void
    {
        if ($dry) {
            $this->line("[dry] редирект {$from} → {$to} (301).");
        ...
        Redirect::updateOrCreate(
            ['from_path' => $from],
            ['to_url' => $to, 'status_code' => 301, 'comment' => 'Старый адрес: '.$what],
        );
        $this->line("Редирект {$from} → {$to} (301).");
    }
